...
\section{Evaluation}
In this section, we will evaulate different parts of our design/implementation,
...
necessary, then it should be limitied to the places where it is strictly
...
the tree, futher than your own supervisor. For example, there's no way, as an
...
\item {\bf Supervision trees: } Supervision trees can be defined in a terse and
  ...
  \texttt{ChildSpec}s. Tjeu can be treated as data-structures, allowing for
  ...
  further down the supervison tree requires futher queries of this kind, which
...
get, while not making it unnecessarily restrictive. It is fairly easy to setup
supervision trees, while still allowing the user to add to its layout
dynamically. There are some areas that are harder use than necessary, but this

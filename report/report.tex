...
\item \textbf{Immutability and actor isolation}\\
  ...
  One of the main benefits and isolation, is to avoid shared mutable state, and
  ...
  of shared and mutable state. This means that the can affect other threads by
  ways other than message passing, however, the common downsides of this are
  avoided.
...
\item \textbf{Monitoring and killing other processes}\\
  ...
  to terminate itself, in a regular message.
\item \textbf{Behaviors}\\
  In Erlang/OTP, the way you use behaviors is to implement a set of callback
...
\item \textbf{Supervision}\\
  ...
  \item \textbf{Supervisor - Interaction}\\
    ...
    child, given a specification, and allows for monitoring children i.e. being
    notified whenever a child is shutdown or restarted. All children are given a
    mailbox to their supervisor, so that they can make these requests of them.
  \end{itemize}
\item \textbf{Behaviours}\\
...
\item {\bf The request-handler behaviour} \\
  ...
  \texttt{Response::NoReply}. When if \texttt{on\_request} returns with a reply,
  said reply is directly sent back to the requester. If no response is returned
  by the request-handler then the reply is deferred by storing request in a map
  ...
  earlier. There no need to communicate a mailbox for the request-handler to
